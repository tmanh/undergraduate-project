...
{
	std::sort(vector1.begin(), vector1.end()/*, greater<double>()*/);
    t = vector1.at(N-1);
}
...
int main(int argc, char* argv[]) {
    ...
    size.width =width;
    size.height=height;
    cout << "depth: " << pix_depth << " - " <<  "Channels: " << nc << endl << endl;
    ...
    cvSaveImage("dwt.bmp",cvImg);
	 ...
	 cout << endl << "Image Reconstruction with 10% of the largest coefficients" << endl;
     int n_coef1 = int (output.size() / 10);
     cout << "Number of coefficients: " << n_coef1 << endl;
	...
	cout << "Image Reconstruction with 2% of the largest coefficients" << endl;
	int n_coef2 = int (output.size() / 50);
	cout << "Number of coefficients: " << n_coef2 << endl;
