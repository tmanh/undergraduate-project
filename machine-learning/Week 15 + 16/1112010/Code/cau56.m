...
function cau56(features_in, features_out)
%model0 model1, model2, model3, model4, err_in, err_out
    disp('Ket qua thi nghiem voi Q = 2');
    [model0 model1, model2, model3, model4, err_in, err_out] = Q2(features_in, features_out);
    disp(' ');
    disp('Thong tin mo hinh svm theo thu tu tang dan cua C(tu 0.0001 den 1):');
    disp(model0); disp(model1); disp(model2); disp(model3); disp(model4);
    disp('Thong tin do loi Ein theo thu tu tang dan cua C(tu 0.0001 den 1):');
    disp(err_in);
    disp('Thong tin do loi Eout theo thu tu tang dan cua C(tu 0.0001 den 1):');
    disp(err_out);
    disp('Ket qua thi nghiem voi Q = 5');
    [model0 model1, model2, model3, model4, err_in, err_out] = Q5(features_in, features_out);
    disp(' ');
    disp('Thong tin mo hinh svm theo thu tu tang dan cua C(tu 0.0001 den 1):');
    disp(model0); disp(model1); disp(model2); disp(model3); disp(model4);
    disp('Thong tin do loi Ein theo thu tu tang dan cua C(tu 0.0001 den 1):');
...
function [model0 model1, model2, model3, model4, err_in, err_out] = Q2(features_in, features_out)
    ...
    model0 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.0001 -d 2 -q');
    model1 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.001 -d 2 -q');
    model2 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.01 -d 2 -q');
    model3 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.1 -d 2 -q');
    model4 = svmtrain(in_label(:, 1), in, '-t 1 -c 1 -d 2 -q');
    
    %phan lop In
    [predicted_label0] = svmpredict(in_label, in, model0);
    [predicted_label1] = svmpredict(in_label, in, model1);
    [predicted_label2] = svmpredict(in_label, in, model2);
    [predicted_label3] = svmpredict(in_label, in, model3);
    [predicted_label4] = svmpredict(in_label, in, model4);
    ...
    %phan lop Out
    [predicted_label0] = svmpredict(out_label, out, model0);
    [predicted_label1] = svmpredict(out_label, out, model1);
    [predicted_label2] = svmpredict(out_label, out, model2);
    [predicted_label3] = svmpredict(out_label, out, model3);
    [predicted_label4] = svmpredict(out_label, out, model4);
...
function [model0 model1, model2, model3, model4, err_in, err_out] = Q5(features_in, features_out)
    %khoi tao nhan training set
    lenght_in = length(features_in);
    lenght_out = length(features_out);
    tmp_ones_in = ones(1, lenght_in);
    ...
    model0 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.0001 -d 5 -q');
    model1 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.001 -d 5 -q');
    model2 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.01 -d 5 -q');
    model3 = svmtrain(in_label(:, 1), in, '-t 1 -c 0.1 -d 5 -q');
    model4 = svmtrain(in_label(:, 1), in, '-t 1 -c 1 -d 5 -q');
